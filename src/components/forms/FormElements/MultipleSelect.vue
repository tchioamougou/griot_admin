...
const toggleItem = (item:any) => {
  const index = selectedItems.value.findIndex((selected:any) => selected.value === item.value)
  if (index === -1) {
...
const isSelected = (item:any) => {
  return selectedItems.value.some((selected:any) => selected.value === item.value)
}
